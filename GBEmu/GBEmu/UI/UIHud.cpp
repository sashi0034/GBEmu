...
namespace GBEmu::UI
{
	...
	void UIHud::DrawLeft(UIEnv& ui, HW::HWEnv& hw, const Point& start, int width, int height)
	{
		...
		text += U"AF={:04X}  BC={:04X}  DE={:04X}  HL={:04X}\n\n"_fmt(cpu.RegAF(), cpu.RegBC(), cpu.RegDE(),
		                                                              cpu.RegHL());
	...
	void UIHud::DrawRight(UIEnv& ui, HW::HWEnv& hw, const Point& start, int width, int height)
	{
		...
		static const auto buttonRight = Rect(buttonPad + Point(buttonSize.x * 2, buttonSize.y), buttonSize).
			rounded(buttonRound);
		static const auto buttonUp = Rect(buttonPad + Point(buttonSize.x, 0), buttonSize).rounded(buttonRound);
		static const auto buttonDown = Rect(buttonPad + Point(buttonSize.x, buttonSize.y * 2), buttonSize).
			rounded(buttonRound);

		constexpr int buttonCircleRound = 20;
		static const auto buttonA = Rect(buttonPad + Point(int(buttonSize.x * 3.5), buttonSize.y * 0), buttonSize).
			rounded(buttonCircleRound);
		static const auto buttonB = Rect(buttonPad + Point(int(buttonSize.x * 3.5), buttonSize.y * 2), buttonSize).
			rounded(buttonCircleRound);

		static const auto buttonSelect = Rect(
			buttonPad + Point(buttonSize.x * 5, buttonSize.y * 0), buttonSize + buttonSize.x0()).rounded(buttonRound);
		static const auto buttonStart = Rect(
			buttonPad + Point(buttonSize.x * 5, buttonSize.y * 2), buttonSize + buttonSize.x0()).rounded(buttonRound);
	...
	void UIHud::DrawUp(
		UIEnv& ui, HW::HWEnv& hw, const HW::HWFrame& hwFrame, const Point& bottom, const Point& displayTop)
	{
		auto&& font = UIAsset::Instance().FontDotGothic18;
		...
		if (hwFrame.IsSuspended())
		{
			...
			(void)Rect(displayTop.movedBy(-border.x / 2, -border.y), border).rounded(4).draw(
				ConstParam::ColorGamingGreen);
