...
namespace GBEmu::HW
{
	...
	struct CPUInstructionProperty
	{
		...
		Optional<CPUInstruction> CodeUnprefixed() const
		{
			return IsPrefixedCB ? none : Optional(static_cast<CPUInstruction>(Code));
		}

		Optional<CPUInstructionCB> CodePrefixedCB() const
		{
			return IsPrefixedCB ? Optional(static_cast<CPUInstructionCB>(Code)) : none;
		}
	...
	private:
		...
		Optional<CPUCycle> handleInterrupt(
	...
	uint8 CPU::GetReg8(CPUReg8 kind) const
	{
		...
		default: HWLogger::Error(U"attempt to get invalid register");
			return 0;
		}
	}
	...
	void CPU::SetReg8(CPUReg8 kind, uint8 value)
	{
		switch (kind)
		{
		case CPUReg8::A: SetA(value);
			return;
		case CPUReg8::F: SetF(value);
			return;
		case CPUReg8::B: SetB(value);
			return;
		case CPUReg8::C: SetC(value);
			return;
		case CPUReg8::D: SetD(value);
			return;
		case CPUReg8::E: SetE(value);
			return;
		case CPUReg8::H: SetH(value);
			return;
		case CPUReg8::L: SetL(value);
			return;
		default: HWLogger::Error(U"attempt to set invalid register");
			return;
		}
	}
}
