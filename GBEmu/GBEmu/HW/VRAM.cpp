...
namespace GBEmu::HW
{
	...
	void VRAM::refreshAtlas()
	{
		...
		for (uint16 addr = TileDataTableStart_0x8000; addr <= BGAndWindowTileData2End_0x97FF; addr +=
		     singleTileBytes_0x10)
		{
			...
			for (uint8 v = 0; v <= 7; ++v)
			{
				...
				{
					const uint8 u0 = 7 - u;
					const int paletteIndex = ((tileLineData >> u0) & 0b1) | (((tileLineData >> (u0 + 8)) & 0b1) << 1);
					// 0 ~ 3
